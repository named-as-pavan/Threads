...
import { Box, Button, Flex, FormControl, Input, Text, useDisclosure } from '@chakra-ui/react';
...
const Actions = ({post}) => {
  ...
  const handleReply = async()=>{
    if(!user){
      showToast("Error","Please login to interact with posts","error")
      return;
    }
    setIsReplying(true)
    ...
    try {
      const res = await fetch(`/api/posts/reply/${post?._id}`,{
        ...
        body: JSON.stringify({text:reply,id:user._id})
      })
  ...
  const handleLikeAndUnlike = async () => {
    if(!user){
      showToast("Error",'you must login to interact with posts',"error")
      return
    }
    if(isLiking) return;
    setIsLiking(true)
